...
flags.DEFINE_integer('use_gt_skin', 0, 'whether using gt or not')
flags.DEFINE_string('gt_animal','aardvark_female','initial animal shape')
...
class LASR(MeshNet):
    def __init__(self, input_shape, opts, nz_feat=100):
        ...
        # if opts.use_gt_skin:
        #     self.rest_rs = torch.Tensor([[0, 0, 0, 1]]).repeat(opts.n_bones , 1).cuda()
        #     self.transg = torch.Tensor([[0, 0, 0]]).cuda().repeat(opts.n_bones ,
        #                                                           1)  # not including the body-to-world transform
        #
        #     self.ctl_rs =  torch.Tensor([[0,0,0,1]]).repeat(opts.n_hypo*(opts.n_bones),1).cuda()
        #     self.rest_ts = torch.zeros(opts.n_hypo*(opts.n_bones),3).cuda()
        #     self.ctl_ts =  torch.zeros(opts.n_hypo*(opts.n_bones),3).cuda()
        #     self.log_ctl = torch.Tensor([[0,0,0]]).cuda().repeat(opts.n_hypo*(opts.n_bones),1)  # control point varuance
        #
        # else:

        self.rest_rs = torch.Tensor([[0, 0, 0, 1]]).repeat(opts.n_bones - 1, 1).cuda()
        self.transg = torch.Tensor([[0, 0, 0]]).cuda().repeat(opts.n_bones - 1,
                                                              1)  # not including the body-to-world transform
...
#        start_time = time.time()
        ...
        # pdb.set_trace()
        # if opts.use_gt_skin:
        #     skin = np.load('tmp/{}/W1.npy'.format(opts.gt_animal))
        #     skin = torch.tensor(skin).permute(1,0)[None, :, :, None].cuda()
        #
        ...
        #     # pdb.set_trace()
        #     rest_ts = self.rest_ts[:, None, :, None].repeat(local_batch_size * 2, 1, 1, 1).view(-1, opts.n_bones, 3,
        #                                                                                         1)  # bh,m,3,1
        #     ctl_ts = self.ctl_ts[:, None, :, None].repeat(local_batch_size * 2, 1, 1, 1).view(-1, opts.n_bones, 3,
        #                                                                                       1)  # bh,m,3,1
        #
        ...
        #
        #     # pdb.set_trace()
        #     Tmat = -Rmat.matmul(ctl_ts) + Tmat + ctl_ts
        #     Rmat = Rmat.permute(0, 1, 3, 2)
        #     Rmat = Rmat.view(-1, 3, 3)
        #     Tmat = Tmat.view(-1, 3)
        #
        #     # pdb.set_trace()
        #     self.joints_proj = obj_to_cam(rest_ts[:, :, :, 0], Rmat.detach(), Tmat[:, np.newaxis].detach(),
        #                                   opts.n_bones, opts.n_hypo,
        #                                   torch.eye(opts.n_bones)[None, :, :, None].cuda())
        #     self.joints_proj = pinhole_cam(self.joints_proj, ppoint.detach(), scale.detach())
        #     self.ctl_proj = obj_to_cam(ctl_ts[:, :, :, 0], Rmat.detach(), Tmat[:, np.newaxis].detach(), opts.n_bones,
        #                                opts.n_hypo, torch.eye(opts.n_bones)[None, :, :, None].cuda())
        #     self.ctl_proj = pinhole_cam(self.ctl_proj, ppoint.detach(), scale.detach())
        #
        #     # pdb.set_trace()
        #     self.deform_v = obj_to_cam(pred_v, Rmat.view(-1, 3, 3), Tmat[:, np.newaxis, :], opts.n_bones, opts.n_hypo,
        #                                skin, tocam=False)
