...
def render_flow(renderer, verts, faces, verts_pos0, verts_pos1, pp0, pp1, proj_cam0, proj_cam1, h, w):
    ...
    verts_pre[:, :, 1] = -1 * verts_pre[:, :, 1]
    verts_pos0_px = renderer.render_mesh(
        sr.Mesh(verts_pre, faces, textures=verts_pos0[:, :, :3], texture_type='vertex'))[:, :3].permute(0, 2, 3, 1)
    verts_pos1_px = renderer.render_mesh(
        sr.Mesh(verts_pre, faces, textures=verts_pos1[:, :, :3], texture_type='vertex'))[:, :3].permute(0, 2, 3, 1)
    ...
    verts_pos0_px[:, :, :, 1] = pp0[:, 1:2, np.newaxis] + verts_pos0_px[:, :, :, 1].clone() * proj_cam0[:, :1,
                                                                                              np.newaxis] / verts_pos0_px[
                                                                                                            :, :, :,
                                                                                                            2].clone()
    verts_pos0_px[:, :, :, 0] = pp0[:, 0:1, np.newaxis] + verts_pos0_px[:, :, :, 0].clone() * proj_cam0[:, :1,
                                                                                              np.newaxis] / verts_pos0_px[
                                                                                                            :, :, :,
                                                                                                            2].clone()
    verts_pos1_px[:, :, :, 1] = pp1[:, 1:2, np.newaxis] + verts_pos1_px[:, :, :, 1].clone() * proj_cam1[:, :1,
                                                                                              np.newaxis] / verts_pos1_px[
                                                                                                            :, :, :,
                                                                                                            2].clone()
    verts_pos1_px[:, :, :, 0] = pp1[:, 0:1, np.newaxis] + verts_pos1_px[:, :, :, 0].clone() * proj_cam1[:, :1,
                                                                                              np.newaxis] / verts_pos1_px[
                                                                                                            :, :, :,
                                                                                                            2].clone()
...
for i in range(args.nframes):
    if args.model == 'spot':
        mesh = sr.Mesh.from_obj('database/misc/spot/spot_triangulated.obj', load_texture=True, texture_res=5,
        ...
        mesh = sr.Mesh.from_obj('/data/gengshany_google_com/Downloads/eagled/Eagle-original_%06d.obj' % 0,
        ...
        mesh = sr.Mesh.from_obj('/home/gengshany_google_com/Downloads/dogd/dog_model_animation_%06d.obj' % (xtime * i),
                                load_texture=True, texture_res=5, texture_type='surface')
        overts = mesh.vertices
        overts[:, :, 1] *= -1
        overts[:, :, 1] += 0.4
        ...
        mesh = sr.Mesh.from_obj('/home/gengshany_google_com/Downloads/horse3d/Horse_Rigged_%06d.obj' % (xtime * i + 1),
                                load_texture=True, texture_res=5, texture_type='surface')
        overts = mesh.vertices
        overts *= 0.5
        overts[:, :, 2] *= -1
        ...
        mesh = sr.Mesh.from_obj('/home/gengshany_google_com/Downloads/stone/stone_%06d.obj' % (i * xtime),
                                load_texture=True, texture_res=5, texture_type='surface')
        overts = mesh.vertices
        overts[:, :, 1] *= -1
        overts[:, :, 1] += 4.5
...
for i in range(0, args.nframes):
    ...
    verts[:, :, 2] = (
                (verts[:, :, 2] - verts[:, :, 2].min()) / (verts[:, :, 2].max() - verts[:, :, 2].min()) - 0.5).detach()
...
for i in range(0, args.nframes):
    # render flow
    if (i - dframe) >= 0:
        flow_fw, bgmask_fw = render_flow(renderer, verts_list[i - dframe], faces, verts_pos_list[i - dframe],
                                         verts_pos_list[i], pp, pp, cam_list[i], cam_list[i], img_size, img_size)
        ...
        flow_bw, bgmask_bw = render_flow(renderer, verts_list[i], faces, verts_pos_list[i], verts_pos_list[i - dframe],
                                         pp, pp, cam_list[i], cam_list[i], img_size, img_size)
        ...
        util_flow.write_pfm('%s/DAVIS/FlowFW/Full-Resolution/%s/flo-%05d.pfm' % (filedir, args.outdir, i - dframe),
                            flow_fw)
        util_flow.write_pfm('%s/DAVIS/FlowBW/Full-Resolution/%s/flo-%05d.pfm' % (filedir, args.outdir, i), flow_bw)
        util_flow.write_pfm('%s/DAVIS/FlowFW/Full-Resolution/%s/occ-%05d.pfm' % (filedir, args.outdir, i - dframe),
                            occ_fw)
        util_flow.write_pfm('%s/DAVIS/FlowBW/Full-Resolution/%s/occ-%05d.pfm' % (filedir, args.outdir, i), occ_bw)
        cv2.imwrite('%s/DAVIS/FlowFW/Full-Resolution/%s/col-%05d.jpg' % (filedir, args.outdir, i - dframe),
                    flowlib.flow_to_image(flow_fw)[:, :, ::-1])
        cv2.imwrite('%s/DAVIS/FlowBW/Full-Resolution/%s/col-%05d.jpg' % (filedir, args.outdir, i),
                    flowlib.flow_to_image(flow_bw)[:, :, ::-1])