...
parser.add_argument('--datapath', default='/projects/perception/datasets/animal_videos/version9/',
                    help='dataset path')
parser.add_argument('--animal',default='african_elephant_female',type=str,
                    help='animal')
...
def main(animal='african_elephant_female'):

    data_info_path = os.path.join(args.datapath, animal, 'info')

    test_left_img = sorted(glob.glob('%s/*.png' % (data_info_path)))
    # silhouettes = sorted(glob.glob('%s/*'%(args.datapath.replace('JPEGImages', 'Annotations'))))

    silhouettes = sorted(glob.glob('%s/*.npz' % (data_info_path)))


    model.eval()
    # while True:
    out_dir = os.path.join('database',animal)
    os.makedirs(out_dir,exist_ok=True)
    os.makedirs(os.path.join(out_dir,'FlowFW'),exist_ok=True)
    os.makedirs(os.path.join(out_dir,'FlowBW'),exist_ok=True)
    os.makedirs(os.path.join(out_dir,'Images'),exist_ok=True)
    os.makedirs(os.path.join(out_dir,'Mask'),exist_ok=True)


    for inx in tqdm(range(0,180-1)):
        ...
        mask = np.load(silhouettes[inx])['segmentation_masks'] > 0
        maskR = np.load(silhouettes[jnx])['segmentation_masks'] > 0
        ...
        if medflow > args.flow_threshold:
            ...
            np.save('{}/FlowFW/flo-{:05d}.npy'.format(out_dir,inx),flowfw[::-1].astype(np.float32))
            np.save('{}/FlowFW/occ-{:05d}.npy'.format(out_dir, inx), occfw[::-1].astype(np.float32))
            np.save('{}/FlowBW/flo-{:05d}.npy'.format(out_dir,inx+1),flowbw[::-1].astype(np.float32))
            np.save('{}/FlowBW/occ-{:05d}.npy'.format(out_dir, inx+1), occbw[::-1].astype(np.float32))

            cv2.imwrite('{}/Images/{:05d}.png'.format(out_dir,inx), imgL_o[:,:,::-1])
            cv2.imwrite('{}/Images/{:05d}.png'.format(out_dir,inx+1), imgR_o[:,:,::-1])
            # cv2.imwrite('{}/Mask/{:05d}.png'.format(out_dir,inx), mask.astype(np.uint8))
            # cv2.imwrite('{}/Mask/{:05d}.png'.format(out_dir,inx+1 ), maskR.astype(np.uint8))
            # pdb.set_trace()
            np.save('{}/Mask/{:05d}.npy'.format(out_dir,inx),mask.astype(np.uint8))
            np.save('{}/Mask/{:05d}.npy'.format(out_dir,inx+1),maskR.astype(np.uint8))
        #     inx=jnx
        #     ix+=1
        # jnx+=1
